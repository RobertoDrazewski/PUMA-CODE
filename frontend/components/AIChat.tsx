      ...
      <div className="relative z-[10000] w-full max-w-md mx-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {step === 'welcome' && (
          <div className="bg-gray-900 border border-white/10 p-10 rounded-[3rem] text-center shadow-2xl relative animate-in zoom-in duration-300">
            <button onClick={onClose} title={t.chat_success_close} className="absolute top-6 right-6 text-gray-500 hover:text-white">
          ...
          <div className="h-[85vh] md:h-[650px] bg-black flex flex-col rounded-[2.5rem] border border-white/10 overflow-hidden shadow-2xl relative animate-in slide-in-from-bottom-4">
            <div className="flex justify-between items-center bg-gray-900 px-6 py-4 border-b border-white/5">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={onClose} title={t.chat_success_close} className="p-2 text-gray-500 hover:text-white">
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-6 bg-black custom-scrollbar scroll-smooth">
              ...
              {loading && <div className="text-blue-500 text-[10px] font-black animate-pulse uppercase ml-2 tracking-widest">
                {lang === 'es' ? 'Puma pensando...' : lang === 'pt' ? 'Puma pensando...' : 'Puma thinking...'}
              </div>}
              {messages.length >= 6 && (
            ...
            <div className="p-4 bg-gray-900 border-t border-white/5 pb-safe flex gap-2 items-center">
                ...
                <button onClick={sendMessage} title={t.chat_button_send} className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white active:scale-95 shadow-lg shadow-blue-500/20"><svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z" /></svg></button>
            </div>
          </div>
        )}

        {/* ÉXITO (MODIFICADO CON TRADUCCIONES COMPLETAS) */}
        {step === 'success' && (
          <div className="bg-gray-900 border border-white/10 p-10 rounded-[3rem] text-center shadow-2xl relative animate-in zoom-in duration-300">
            <button onClick={onClose} title={t.chat_success_close} className="absolute top-6 right-6 text-gray-500 hover:text-white">
              <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12" /></svg>
            ...
            <h2 className="text-2xl font-black text-white uppercase mb-4 tracking-tighter">
              {t.chat_success_title}
            </h2>
            <p className="text-gray-400 text-sm leading-relaxed mb-10 px-2">
              {t.chat_success_desc}
            </p>
            <button onClick={onClose} className="w-full py-4 bg-blue-600 text-white font-black rounded-2xl uppercase tracking-widest active:scale-95 shadow-lg shadow-blue-500/20">
