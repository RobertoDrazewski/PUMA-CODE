...
export default function Home() {
  ...
  // REPARACIÓN DE SCROLL: 
  // Este efecto asegura que el scroll se habilite/deshabilite correctamente
  useEffect(() => {
    if (showChat) {
    ...
    <main className="relative min-h-screen bg-black text-white overflow-x-hidden flex flex-col selection:bg-blue-500/30">
      ...
      <section id="services" className="py-32 px-6 max-w-7xl mx-auto w-full border-t border-white/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[1, 2, 3, 4, 5, 6].map((num, index) => (
            <div key={num} className="group p-10 rounded-[2.5rem] bg-gradient-to-br from-white/5 to-transparent border border-white/10 hover:border-blue-500/50 transition-all duration-500 hover:-translate-y-3">
              <div className="text-5xl mb-6 grayscale group-hover:grayscale-0 transition-all duration-500">{serviceIcons[index]}</div>
              <h3 className="text-2xl font-bold mb-4 group-hover:text-blue-400 transition-colors">{t[`s${num}_title`]}</h3>
              <p className="text-gray-400 leading-relaxed text-sm">{t[`s${num}_desc`]}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="contact" className="py-32 px-6 bg-gradient-to-t from-blue-900/10 to-black border-t border-white/5">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-5xl md:text-7xl font-black mb-12 tracking-tighter">{t.contact_title}</h2>
          <div className="flex flex-wrap justify-center items-center gap-6 md:gap-8">
            <a href="mailto:info@puma-code.com" className="group flex items-center gap-4 px-8 py-5 bg-white text-black font-black rounded-3xl hover:bg-blue-600 hover:text-white transition-all duration-500 shadow-2xl active:scale-95">
            ...
            <a href="https://www.linkedin.com/in/robert-drazewski" target="_blank" rel="noopener noreferrer" className="group flex items-center gap-4 px-8 py-5 bg-[#0077b5] text-white font-black rounded-3xl hover:bg-[#005582] transition-all duration-500 shadow-2xl active:scale-95">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
              LinkedIn
            </a>
            <a href="https://www.instagram.com/puma_code" target="_blank" rel="noopener noreferrer" className="group flex items-center gap-4 px-8 py-5 bg-gradient-to-tr from-[#f09433] via-[#e6683c] via-[#dc2743] to-[#bc1888] text-white font-black rounded-3xl hover:scale-105 transition-all duration-500 shadow-2xl active:scale-95">
